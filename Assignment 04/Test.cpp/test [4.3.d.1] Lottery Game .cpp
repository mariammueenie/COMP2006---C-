...
// use vectors for storing lottery numbers
...
// function collects the user's lottery ticket
// allows either manual entry or a randomly generated ticket
...
// function generates a random bonus line of 6 unique numbers
// reuses the generateUnique helper and keeps the code modular
vector<int> generateRandomLine() {
    return generateUnique(6, 1, 49);
}

// compares a player's line to the winning numbers
// counts how many matches there are and also checks if the bonus number matches
// bonus number counts as one extra match
int compareLines(const vector<int>& line, const vector<int>& winning, int bonusNum) {
    int matches = 0;

    // Ccheck each number in the player's line for a match with the main winning numbers
    for (int n : line) {
...
// determines prize money based on the number of matches
// follows the specific prize rules defined in the assignment
...
// function writes the ticket, bonus line, winning numbers, and prize results
...
// function brings together all the lottery logic
...
void playLotteryGame() {
    ...
    // outer loop allows the player to play multiple rounds of the lottery
...
// CHANGES TO MAKE 
// so many. dumb code does not work
// validate every input in every way possible >:( 
